...
;
; CALLING SEQUENCE:
;      IDL> holo = dbr_spheredhm(rp, ap, np, nm, dim)
;
; INPUTS:
...
;
;      IDL> tvscl, dbr_spheredhm([0,0,200],0.75,1.5,1.33,[201,201])
;
...
function dbr_spheredhm, rsphere, a, np, nm, dim, alpha = alpha, $
...
field = dbr_spherefield(x, y, zp, a, np, nm=nm, $
                  mpp=mpp, lambda=lambda, precision=precision, $
