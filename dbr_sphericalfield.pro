...
;            New York University
; 3/21/2014: Using Jackson vector spherical harmonics
;
...
;Calculate the coefficients 
      if m eq 1 then begin
         am_mn = - ci^n*sqrt(4.d*!pi*(2.d*n + 1.d ))/2.d
         ae_mn = -ci*am_mn
      endif else begin
         am_mn = - ci^n*sqrt(4.d*!pi*(2.d*n + 1.d ))/2.d
         ae_mn = ci*am_mn
...
;    M1n[0,*] = 0.d
      Mmn[1,*] = -xi_n*pi_mn*exp(ci*m*phi)/sqrt(n*(double(n)+1))
; ... divided by 1/kr
      Mmn[2,*] = -ci*xi_n*tau_mn*exp(ci*m*phi)/sqrt(n*(double(n)+1)) 
; ... divided by 1/kr

      Nmn[0,*] = -xi_n*p_mn*exp(ci*m*phi)*sqrt(n*(double(n)+1))
