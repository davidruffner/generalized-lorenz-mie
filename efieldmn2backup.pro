...
dylmdtheta = dbr_sphericalharmonicprime(theta,phi,n,m)
...
if b1 ne !NULL then begin 
   print,"we have nansss! in efieldmn2"
   stop
endif
...
Ethmn = -Amn*psiprime*dylmdtheta/(kr*sqrt(n*(double(n)+1))) $
        -Bmn*m*psin*ylm/(kr*sqrt(n*(double(n)+1))*sintheta)

Ephimn = -Amn*im*m*psiprime*ylm/(kr*sintheta*sqrt(n*(double(n)+1))) $
         -Bmn*im*psin*dylmdtheta/(kr*sqrt(n*(double(n)+1)))
